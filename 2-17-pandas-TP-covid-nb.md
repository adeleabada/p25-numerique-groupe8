...
```{code-cell} ipython3
#on regarde le nombre de valeurs dans une des colonnes, 'date' par ex
fr['date'].value_counts()
len(fr['date'].value_counts())
```
...
full_df = pd.concat(liste, ignore_index=True)
full_df
```
...
```{code-cell} ipython3
# version avec pivot
clean_df = pd.pivot_table(global_df, values=['confirmed','deaths','recovered'], index=['country', 'date'])
...
```{code-cell} ipython3
clean_df.loc[('France',)].plot() #on reutilise ce qu'on a fait dans la partie multinindex 
```
...
```{code-cell} ipython3
clean_df.loc['France'][['confirmed','deaths']].plot() #on reslice 
```
...
df3.plot(rot=45)
#on veut un index simple, on aura donc 6colonnes
```

```{code-cell} ipython3
:tags: [raises-exception]

us_df3= df3.unstack(level=0) 
us_df3 
us_df3.plot(rot=45)
#on est repassé a un index simple
```
...
def extract(clean_df, countries, measures, start_date, end_date):
    # on selectionne les pays et ce qu'on veut mesurer 
    df_extract = clean_df.loc[countries, measures].reset_index()  # on remet la date comme index 
    df_extract['date'] = pd.to_datetime(df_extract['date'])  # on convertit
    df_extract = df_extract[(df_extract['date'] >= pd.to_datetime(start_date)) & 
                            (df_extract['date'] <= pd.to_datetime(end_date))]
    df_extract = df_extract.set_index(['date','country']).sort_index()
    
    df_extract = df_extract.unstack(level=1)
    df_extract.columns = [f"{country}_{measure}" for measure, country in df_extract.columns]  # boucle pour nommer les colonnes pays_mesure 
    df_extract = df_extract.apply(pd.to_numeric)  # s'assurer que les colonnes sont numériques
...
```{code-cell} ipython3
df_extract = extract(clean_df, ['France','Italy'], ['confirmed','deaths'], '2021-01-01', '2021-08-31')
df_extract.plot(figsize=(12,6), rot=45, title=f'COVID {measures} : {", ".join(countries)}')
...
```{code-cell} ipython3
 #j'ai decouvert la possibilité de faire des multiIndex et comment l'indexation eatit importante pour simplifier les codes 
```
