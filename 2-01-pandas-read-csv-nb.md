...
```{code-cell} ipython3
df['Age'].describe() # sans double crochet pas le mm affichage 
```
...
```{code-cell} ipython3
Les problèmes sont les points virgules
il y a problème de header il y a des données a la place du nom des colonnes 
3eme erreur ?
```
...
```{code-cell} ipython3
colonnes = ['PassengerId', 'Survived', 'Pclass', 'Name', 'Sex', 'Age', 'SibSp',
          'Parch', 'Ticket', 'Fare', 'Cabin', 'Embarked']
df = pd.read_csv("data/petit-titanic.csv", sep=";", header=None, names=colonnes )
df.head()
```
...
```{code-cell} ipython3
# en creeant un nouveau dataframe avec les colonnes voulues
df_Age_Pclass = df[["Age","Fare"]]
df_Age_Pclass.describe()
```
...
```{code-cell} ipython3
len(df.columns)
#Si on fais juste len(df) ca nous donne les lignes df.shape nous donne les lignes et les colonnes 
```
...
```{code-cell} ipython3
df.columns=['Identifiant', 'Survécu', 'Pclass',
           'Nom', 'Genre', 'Age', 'SibSp', 'Parch',
           'Ticket', 'Tarif', 'Cabine', 'Embarquement']
df
```
